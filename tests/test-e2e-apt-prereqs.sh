...
apt_prereq_case() { # <failed attempts> -> transcript
    : >"$T449/attempts"
    : >"$T449/sleeps"
    E2E_APT_TRIES='' APT_FAIL_FOR="$1" APT_ATTEMPTS="$T449/attempts" APT_SLEEPS="$T449/sleeps" ROOT="$ROOT" PATH="$T449/bin:$PATH" bash -c '
        sleep() { printf "%s\n" "$1" >>"$APT_SLEEPS"; }
...
assert_eq "e2e: apt success stops after its first attempt (#449)" "$(printf '%s\n' "$out" | tail -1)" "OK attempts=1 sleeps="
out="$(apt_prereq_case 1)"
assert_eq "e2e: apt retries once, then breaks on success (#449)" "$(printf '%s\n' "$out" | tail -1)" "OK attempts=2 sleeps=5"
assert_contains "e2e: apt retry notice names the attempt and bound (#449)" "$out" "attempt 1/3"
out="$(apt_prereq_case 3)"
assert_eq "e2e: apt failure spends the three-attempt bound (#449)" "$(printf '%s\n' "$out" | tail -1)" "ABORT attempts=3 sleeps=5,10"
