...
### D13. A masked secret is a marker the applier resolves, not an absence the consumer must fill (#415)

D10 masks pool secrets on the read feed, and until #415 it masked them by *deleting* the key. Deleting the value was right. Deleting the fact that a value is set was not: `pools` is a D3 writable key, jq's `*` replaces arrays wholesale, and `parse_config` defaults a missing `pass` to XMRig's throwaway `x` — a value that passes every check. So the only `pools` array a consumer of the feed could construct was one with no `pass`, and sending it re-keyed the rig to `x`, reported `applied`, and logged nothing. The consumer could not work around this alone: it is the one party that never holds the value, and it had no signal that a value existed, leaving it a choice between wiping the credential on every pools edit and refusing pools edits entirely.

Decision: a set secret is served as the `{"__secret__": true}` sentinel — the marker Pithead's Configuration view already uses, so the two sides share one convention — and `_control_commit` resolves it back to the stored value **before** the overlay. An omitted key resolves the same way, because that is what an older consumer sends. The identity for the lookup is the `(url, user)` pair, first-declared winning on a duplicate, mirroring how Pithead restores its per-worker token sentinels: a pool credential authenticates an *account* at a *host*, so carrying a password across a changed url or user would be a second silent bug rather than a fix for this one. An unset secret is still absent, so "no marker" reliably means "nothing here to keep".

Two properties are deliberate. A sentinel that resolves to nothing is **rejected** (`unresolvable-secret-marker`) rather than dropped — it asks to keep something that is not there, and this whole decision exists because a credential must never change quietly. And an explicit value is always taken at face value, so a real new password still replaces the old one, `""` still hits #408's rejection, and clearing a key stays possible. The resolver is not the guarantee, though: `parse_config` now rejects a non-string `pass` outright, so a marker that reaches `config.json` by a hand edit or a future writer that forgets to resolve one can never become the rig's literal password. (It could: `jq -r` renders an object as its JSON text, and `{}` is two graph characters on one line, which passed the charset rule.)
