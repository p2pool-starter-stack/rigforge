...
### D8. Sandbox both units; treat the spool file as untrusted input

Both new units carry the full systemd hardening baseline (`NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome`, `PrivateTmp`, `PrivateDevices`, `ProtectKernel*`, `ProtectControlGroups`, `ProtectClock`, `RestrictSUIDSGID`, `RestrictRealtime`, `LockPersonality`, `RestrictNamespaces`, `SystemCallArchitectures=native`, `SystemCallFilter=@system-service`, `RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6`, `UMask=0077`), validated with `systemd-analyze security`. The unprivileged receiver additionally keeps `MemoryDenyWriteExecute=yes` (plain CPython, no JIT). The root builder **omits `MemoryDenyWriteExecute`** (it breaks the C++ compiler/linker) and runs with an **empty `CapabilityBoundingSet`** — the non-obvious win from the review: restarting the miner unit talks to PID 1 over its private socket and needs uid 0, not a capability, and compiling needs no capability either, so a root unit that must build and restart can still drop every capability. `SystemCallFilter` stays the coarse `@system-service` group, never a hand-rolled allowlist (a build spawns cc1/as/ld/collect2 and would whack-a-mole).
