...
It comes from the **verb**, which treats the staged file as untrusted input: `systemd.path` validates nothing, so the oneshot's first act is to refuse a symlink and `rename()` the intent into a root-owned `processing/` dir the `DynamicUser` cannot write (atomically closing the swap/TOCTOU window), then parse it as a claim — a single whitelisted field (`version` matching `^v[0-9]+\.[0-9]+\.[0-9]+$`), never sourced/evaled — before it does anything. That plus the D4/D6/D10 validation (latest-or-refuse target bounds, throttle, reachability, anti-rollback) is where this path is actually defended.
