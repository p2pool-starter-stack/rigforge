...
roughly `L3 size ÷ 2 MB`. This is the math XMRig's auto thread-sizing uses (and the
...
> so it gets thread placement right for CPUs a static table would miss or mishandle. Take **dual-CCD
