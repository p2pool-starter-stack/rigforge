...
| `ACCESS_TOKEN` | `""` *(open)* | Optional bearer token for the XMRig HTTP API and RigForge APIs. Unset leaves the read APIs open. When set, `:8080` and writable `:8082` require the exact bearer; read-only `:8081` also accepts its scoped HMAC-derived read bearer for Pithead 2.0. See [Pithead Integration](pithead-integration.md). |
