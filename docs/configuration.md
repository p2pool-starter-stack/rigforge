...
| `api_allow_from` | `""` *(off)* | Restrict the read-only API port(s) — `:8080`, plus `:8081` when the sister API is on — to a single IPv4 address or CIDR (plus loopback), via an own nftables table. SSH and mining are never touched. Needs `nft`; Linux-only. Layers under the Pithead contract, which is unchanged. |
...
| `max_temp_c` | `""` *(off)* | Optional thermal cutoff for the watchdog: above this °C the miner is stopped, and started again once the rig cools 5 °C below it (fixed hysteresis). Empty = no thermal logic. Check what `/sys/class/thermal/thermal_zone0/temp` actually reports on your board before setting it — the zone's meaning varies (`THERMAL_ZONE` overrides the path). Accepts 40–110. |
