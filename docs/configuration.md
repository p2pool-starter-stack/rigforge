...
| `autotune` | `"disabled"` | Periodic live tuning, as a target: `"disabled"` (default) installs no timer; `"performance"` schedules a nightly tune for **raw hashrate**; `"efficiency"` schedules one for **hashrate-per-watt** (needs a power source — built-in RAPL or `TUNE_POWER_CMD` — else it falls back to `performance` with a warning). Legacy booleans still parse (`true` → `performance`, `false` → `disabled`). See [Operations › Live auto-tuning](operations.md#live-auto-tuning-opt-in). |
